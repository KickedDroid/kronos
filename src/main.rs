...
async fn main() {
    ...
    let toml_string = fs::read_to_string("config.toml").await.expect("No config.toml file found");
    ...
    loop {
        ...
        if content.len() == TARGET_PHRASE.len() && content != last_content {
            ...
            if is_valid_hex(&content) && !auto {
                ...
                let _=stdout().flush();
                stdin().read_line(&mut s).expect("Please Enter a valid string");
...
async fn submit_flag(flag: &str, config: &Config) -> bool {
    ...
    headers.insert("Authorization", format!("Bearer {}", config.htb["api_token"]).parse().expect("Failed to read config.toml"));
    headers.insert("Content-Type", "application/json".to_string().parse().expect("Failed to parse Content-Type"));
